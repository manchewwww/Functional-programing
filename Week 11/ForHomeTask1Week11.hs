...
setLevels tree = set tree 0
  where
    set Nil _ = Nil
    set (Node x left right) level = Node (level, x) (set left (level + 1)) (set right (level + 1))

areEqual :: (Eq a) => BTree a -> BTree a -> Bool
areEqual Nil Nil = True
areEqual Nil _ = False
areEqual _ Nil = False
areEqual (Node value left right) (Node value2 left2 right2) = value == value2 && areEqual left left2 && areEqual right right2
...
average x = roundTwoDigButWithMagic $ (fromIntegral $ sumTree x) / (fromIntegral  $ size x)
...
height :: BTree a -> Int
height Nil = 0
height (Node _ left right) = 1 + height left
